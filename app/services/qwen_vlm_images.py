...
async def vlm_rank_clip_pool(
    ...
    *,
    ...
    """
    rows: (image_url, title, snippet), length = CLIP 后参与 VLM 排序的池子。
    返回长度 n=len(rows) 的下标排列（相关度从高到低），失败返回 None。
    """
    api_key, _, _ = _vlm_env()
    ...
    parts: list[dict[str, Any]] = [
        {
            "type": "text",
            "text": (
                f"你是图像检索裁判。下面按顺序给出 {n} 张候选图，编号 0 到 {n - 1}。\n"
                "请结合用户问题，判断每张图与问题的相关性。\n"
                "只输出一个 JSON 对象，不要其它文字，格式：\n"
                '{"order":[编号按相关程度从高到低排列的整数数组],'
                '"irrelevant":[与问题完全无关的编号数组]}\n'
                f"用户问题：{query}\n"
                "随后每张图对应上述编号顺序（先读说明再看图）。"
            ),
        }
    ]

    blobs = await _fetch_many_image_bytes(
        [(u, d, lp) for (u, _t, d, lp) in rows], timeout=25.0, concurrency=6
    )
    for i in range(n):
        _url, title, desc, _local_path = rows[i]
        parts.append(
            {
                "type": "text",
                "text": f"[候选图{i}] {title or ''} {desc or ''}"[:500],
            }
        )
        blob = blobs[i]
        if blob:
            ...
            parts.append(
                {
                    "type": "text",
                    "text": f"[图{i}无法加载]{title} {desc}"[:800],
                }
            )

    messages = [{"role": "user", "content": parts}]
    try:
        content = await _chat_vlm(messages, max_tokens=900, temperature=0.0, timeout=120.0)
    except Exception:
        logger.exception("vlm_rank_clip_pool_chat_failed")
        return None

    data = _extract_json_obj(content)
    if not data:
        logger.warning("vlm_rank_clip_pool_bad_json content_prefix=%s", content[:200])
        return None
    ...
    irrel: set[int] = set()
    for x in data.get("irrelevant") or []:
        if isinstance(x, int) and 0 <= x < n:
            irrel.add(x)

    order = _complete_index_order(raw_order, n)
    order = [i for i in order if i not in irrel]
    if not order:
        order = list(range(n))
    for i in range(n):
        if i not in order:
